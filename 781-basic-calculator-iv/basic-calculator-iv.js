...
class Term {
  constructor(coefficient = 1, variables = [], degree = 0) {
  ...
  format(variableNames) {
    return !this.coefficient
      ...
      : this.coefficient +
          this.variables
            .map((count, i) => ('*' + variableNames[i]).repeat(count))
            .join('')
  }
}
...
class Polynomial {
  ...
  mul(polynomial) {
    let orig = this.terms.map(t => t.clone())
    this.terms = [] // clear
    for (let term1 of polynomial.terms) {
      for (let term2 of orig) {
        this.addTerm(term1.clone().mul(term2.clone()))
      }
    }
    return this
  }
